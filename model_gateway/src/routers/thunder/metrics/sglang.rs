...
#[derive(Debug)]
pub struct SglangMetricsClient {
    inner: VllmMetricsClient,
}

impl SglangMetricsClient {
...
impl MetricsClient for SglangMetricsClient {
    fn url(&self) -> &str {
        self.inner.url()
    }
    ...
    fn snapshot(&self) -> Value {
        ...
        if let Some(obj) = snapshot.as_object_mut() {
            obj.insert("backend_type".into(), Value::String("sglang".into()));
        }
        snapshot
    }
}
