#!/usr/bin/env bash
# Phase 10 e2e: Thunder SkyRL backend metrics dialect.
...
MOCK_LOG=/tmp/thunder_phase10_skyrl_mock.log
SMG_LOG=/tmp/thunder_phase10_skyrl_smg.log
...
python3 e2e_test/thunder/mock_vllm.py --port "$MOCK_PORT" \
    --kv-cache-block-tokens 8 --num-kv-cache-blocks 64 \
    > "$MOCK_LOG" 2>&1 &
MOCK_PID=$!
...
if [[ -x "$SMG_BIN" ]]; then
    "$SMG_BIN" --backend thunder --worker-urls "http://localhost:$MOCK_PORT" \
        ...
        --thunder-backend-type skyrl > "$SMG_LOG" 2>&1 &
else
